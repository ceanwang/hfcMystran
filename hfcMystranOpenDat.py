...
class hfcMystranOpenDat:
	"Mystran result object"
	def GetResources(self):
		return {"MenuText": "Log In",
				"Accel": "Ctrl+t",
				"ToolTip": "Input log file",
				"Pixmap": os.path.dirname(__file__)+"./resources/dat.svg"
		}
	...
	def Activated(self):
		...
		if iHfc==None:
			...
			try:
				filename = QFileDialog.getOpenFileName(None,QString.fromLocal8Bit("Read a Mystarn's dat file"),path, "*.dat *.dbf") # PyQt4
			except Exception:
				filename, Filter = PySide.QtGui.QFileDialog.getOpenFileName(None, "Read a Mystran's dat file", path, "*.dat *.dbf") #PySide
...
FreeCADGui.addCommand('hfcMystranOpenDat',hfcMystranOpenDat())